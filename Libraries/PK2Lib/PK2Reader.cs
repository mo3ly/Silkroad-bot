namespace PK2Reader
{
    ...
    public class Reader : IDisposable
    {
        ...
        public bool FileExists(string Name)
        {
            try
            {
                Console.WriteLine("PK2 FileExists exception " + Name);
            ...
            {
                Console.WriteLine("PK2 FileExists exception " + e);
                return false;
            }
        }
        ...
        public byte[] GetFileBytes(string Name, string parentOfparent, string parent)
        {
            if (FileExists(Name))
            {
                System.IO.BinaryReader reader = new System.IO.BinaryReader(m_FileStream);
                File file = m_Files.Find(item => item.Name.ToLower() == Name.ToLower() && item.ParentFolder.Name.ToLower() == parent.ToLower() && item.ParentFolder.Path.ToLower() == parentOfparent.ToLower());
                reader.BaseStream.Position = file.Position;
                return reader.ReadBytes((int)file.Size);
            }
            else
                ...
                switch (entry.Type)
                {
                    ...
                    case 1: //Folder 
                        if (entry.Name != "." && entry.Name != "..")
                        {
                            ...
                            folder.Path = m_CurrentFolder.Name;
                    ...
                    case 2: //File
                        ...
                        //if (entry.Name.Contains("ring"))
                            //Console.WriteLine($"Name = {file.Name} \t parent = {m_CurrentFolder.Name} \t parent = {m_CurrentFolder.Path}");
                        break;
                }
