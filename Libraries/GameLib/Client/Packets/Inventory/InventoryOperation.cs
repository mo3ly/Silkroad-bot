    ...
    public class InventoryOperation
    {
        ...
        public static void Parse(Packet p)
        {
            ...
            else if (flag == 0x06) //Item picked from the ground
            {
                ...
                Console.WriteLine("Picked Item slot: " + itemSlot);
                ...
                Information.InventoryItem item = InventoryUtility.PareseOperatedItem(p);
                item.Slot = itemSlot;
                ...
                if (item.MediaName.Contains("RARE") || (item.MediaName.Contains("ROC") && item.MediaName.Contains("SET")))
                {
                    Notifier.ItemNotificaion(item.IconPath); // show this when pet picks an item
                }
            }
            ...
            else if (flag == 0x08) //Item bought from NPC
            {
                ...
                Console.WriteLine(NPCMediaName);
            ...
            else if (flag == 0x18) //Item bought from Item Mall
            {
                //TODO: PARSE PACKET
                //int itemSlot = p.ReadInt8();
                //Console.WriteLine("Mall from Item to slot: " + itemSlot);

                Information.InventoryItem item = InventoryUtility.ParseItemBoughtFromMall(p);
                //item.Slot = itemSlot;
                //item.SubType = Media.Utility.GetSubItemType(item);

                Client.InventoryItems.Add(item.Slot, item); //Re-adds the item with the new info.

                args = new InventoryOperationEventArgs(Client.InventoryItems[item.Slot]);
                args.ItemChangeType = InventoryOperationEventArgs.ChangeType.Inv_ItemBoughtFromMall;
            }
