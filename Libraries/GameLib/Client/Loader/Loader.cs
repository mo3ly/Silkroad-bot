...
namespace SilkroadInformationAPI.Client.Loader
{
    class Loader
    {
        ...
        #region Patches
        string StartingMessageText = "Welcome to the game.";
        string VersionText = $"                                                      Version: 1.{SRCommon.clientVersion}\n                                                      [ds9sd-sd6awd-sd4dd] Loaded successfully.";
        byte[] HexColorArray = { 0xDC, 0xD0, 0xBD, 0x00 };
        byte[] JMP = { 0xEB };
        byte[] RETN = { 0xC3 };
        ...
        private Process StartLoader(string path, ushort port, byte local)
        {
            ...
            SRCommon.SRClientProcess = pSilkroad;
            SRCommon.SRClientID = pSilkroad.Id;
            //SRCommon.ClientTimer.Start();
            ...
            // built in injection -> clear the lib and delete the unnessery
            /*InjectionMethod injector = InjectionMethod.Create(InjectionMethodType.Standard);
            var hModule = IntPtr.Zero;

            using (var dll = new PortableExecutable(SRO_INGAME.Properties.Resources.ClientLib))
                hModule = injector.Inject(dll, SroProcessHandle);
            ...
            //InjectDLL(SroProcessHandle, @"D:\disk\Games\Silkroad\SRO Server files\Client\ClientLib.dll");
