...
namespace SilkroadInformationAPI.Media
{
    class LoadData //TODO: Load files based on location+file name rather than file name only.
    {
                    ...
                    while (dataFile != null)
                    {
                        ...
                        using (System.IO.TextReader streamReader = new System.IO.StringReader(reader.GetFileText(dataFile.ToLower())))
                        {
                            ...
                            while (line != null)
                            {
                                ...
                                //Console.WriteLine(dataFile.ToLower() + " ] itemdata line: " + line);
                                ...
                                if (vars.Length >= 118)
                                {
                                    try
                                    {
                                        ...
                                        if (Used)
                                        {
                                            ...
                                            newItem.SubType = Utility.GetSubItemType(newItem);
                                            newModel.Type = ModelType.Unknown;
                                            newItem.MaxStack = Int32.Parse(vars[57]);
                                            newItem.IconPath = vars[54];
                                            newItem.Level = Int32.Parse(vars[33]);
        ...
        public static void LoadTeleportBuildings()
        {
            ...
            {
                Console.WriteLine("Error loading teleport buildings." + ex.Message);
        ...
        public static void LoadRefRegion()
        {
            try
            {
                using (System.IO.TextReader streamReader = new System.IO.StringReader(reader.GetFileText("refregion.txt"))) // fix this
                {
                    string line = streamReader.ReadLine();
                    ...
                    while (line != null)
                    {
                        ...
                        if (vars.Length >= 6)
                        {
                            try
                            {
                                ushort rID = ushort.Parse(vars[0]); //ushort is not suitable
                            ...
                            {
                                Console.WriteLine("RefRegion error: " + ex.Message);
                            }
                        }
