...
namespace SilkroadInformationAPI.Client.Packets.Character
{
    public class CharacterData
    {
        ...
        private static void Parse(Packet p)
        {
            ...
            Media.Utility.CharacterRace((uint)Client.Info.ModelID); // race
            p.ReadInt8(); //Volume
            ...
            for (int i = 0; i < currentInventoryItems; i++)
            {
                ...
                if (!Client.InventoryItems.ContainsKey(item.Slot))
                    Client.InventoryItems.Add(item.Slot, item);
                else
                    Console.WriteLine("Cannot add his to inv: name {0}, slot {1}", item.MediaName, item.Slot);
            }
            ...
            Console.WriteLine("Angle: " + Client.Position.Angle);
